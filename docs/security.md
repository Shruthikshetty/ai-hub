...
- **Encryption at Rest:** Your API keys are encrypted before being stored in the local database, which reduces plain-text exposure on disk. Note that a compromised logged-in environment can still access app data.
...
When you use fully local providers/configurations (for example, local Ollama/LM Studio models), prompts and outputs can stay on your machine.

If you connect to self-hosted or remote endpoints, privacy and network exposure depend on that deployment.
